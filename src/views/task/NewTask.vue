<template>
    <div class="task-start">
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="60%" :before-close="handleClose">
			...
			<el-form :disabled="this.dialogStatus === 'edit'" label-position="left" rules="formRules" :model="form" inline ref="form">
...
export default {
    ...
    data() {
        ...
        rules: {
          itemNo: [
            { required: true, message: '请输入项目编号', trigger: 'change, blur' }
          ],
          itemName: [
              {required: true,messagr: '请输入项目名称', trigger: 'blur, change'}
          ],
          customerType:[
              {required: true, message: '请选择客户类型', trigger: 'blur, change'}
          ],
          businessType: [
              {reequired: true, message: '请选择业务类型', trigger: 'blur, change'}
          ],
          taskSource: [
              {required: true, message: '请选择任务来源', trigger: 'blur, change'}
          ],
          businessType: [
              {
                  required: true, 
          ...
          buildTime: [
            { type: 'date', required: true, message: '请选择开始', trigger: 'change' }
          ],
          planDate: [
